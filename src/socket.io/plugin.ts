  ...
  return {
    ...
    register: function(server: Server) {
      ...
      io.use(async (socket, next) => {
        ...
        log.debug('websocket.authenticate', socket.data.sessionId)
      ...
      io.on('connection', async function(socket) {
        ...
        socket.on('jitsi-event', async (message) => {

          log.debug('JITSI EVENT', message)

          publish('tokenmeet.live', 'jitsi-event', message)
